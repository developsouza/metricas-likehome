...
export default function AcompanhamentoSetores() {
  ...
  const departamentos = (usuario?.perfil === "admin" ? [...deptos(), "Admin"] : estrategico ? deptos() : [usuario?.departamento].filter(Boolean))
    .sort((a, b) => labelDepto(a).localeCompare(labelDepto(b), "pt-BR"));
  ...
  return <div>
    ...
    {loading ? <div className="loading">Carregando atividades...</div> : <div className="kanban-board">{STATUS.map((status) => { const itens = atividades.filter((a) => a.status === status).sort((a, b) => new Date(`${b.atualizado_em || b.criado_em}Z`) - new Date(`${a.atualizado_em || a.criado_em}Z`)); return <section className="kanban-column" key={status}><header><strong>{status}</strong><span>{itens.length}</span></header><div className="kanban-card-list">{itens.map((a) => <button key={a.id} className={`activity-card priority-${a.prioridade.toLowerCase().replace("é", "e")}${a.atrasada ? " overdue" : ""}`} onClick={() => setModal(a)}><div><span className="priority-label">{a.prioridade}</span>{a.atrasada ? <span className="overdue-label">Atrasada</span> : null}</div><h4>{a.titulo}</h4>{a.descricao && <p>{a.descricao}</p>}<footer><span>{a.responsavel_nome || "Sem responsável"}</span><span>💬 {a.comentarios_total}</span></footer>{a.prazo && <small>Prazo: {new Date(a.prazo).toLocaleString("pt-BR")}</small>}</button>)}</div></section>; })}</div>}
    </>}
    {modal && <ModalAtividade atividade={modal === "nova" ? null : modal} departamento={departamento} podeEditar={modal === "nova" ? podeCriar : podeEditar} podeExcluir={usuario?.perfil === "admin"} onClose={() => { setModal(null); carregar(); }} onSaved={fecharSalvando} />}
  </div>;
}
