...
function ModalUnidade({ unidade, onClose, onSave, empreendimentos, proprietarios, usuarios }) {
  const [form, setForm] = useState(unidade || { empreendimento_id: '', numero: '', tipo: '', status: 'Prospeccao', proprietario_id: '', responsavel_id: '', observacoes: '', data_prospeccao: '', data_reuniao: '', data_fechamento: '', data_integracao: '', data_ativacao: '', data_baixa: '' })
      ...
      <div className="modal-box" style={{ maxWidth: 680 }}>
        ...
        <div className="form-grid">
          ...
          <div className="form-group">
            ...
            <input className="form-control" value={form.numero} onChange={e => handle('numero', e.target.value)} placeholder="Ex: 101, Apto B" />
          </div>
          <div className="form-group">
            <label className="form-label">Tipo</label>
            <select className="form-control" value={form.tipo} onChange={e => handle('tipo', e.target.value)}>
              <option value="">Selecione...</option>
              {['Studio', 'Apartamento 1Q', 'Apartamento 2Q', 'Apartamento 3Q', 'Casa 2Q', 'Casa 3Q', 'Cobertura'].map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div className="form-group">
            <label className="form-label">Status</label>
            <select className="form-control" value={form.status} onChange={e => handle('status', e.target.value)}>
              {STATUS_LIST.map(s => <option key={s} value={s}>{labelStatus(s)}</option>)}
            </select>
          </div>
          <div className="form-group">
        ...
        <div className="form-grid-3">
          <div className="form-group">
            ...
            <input type="date" className="form-control" value={form.data_prospeccao || ''} onChange={e => handle('data_prospeccao', e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Data Reunião</label>
            <input type="date" className="form-control" value={form.data_reuniao || ''} onChange={e => handle('data_reuniao', e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Data Fechamento</label>
            <input type="date" className="form-control" value={form.data_fechamento || ''} onChange={e => handle('data_fechamento', e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Data Integração</label>
            <input type="date" className="form-control" value={form.data_integracao || ''} onChange={e => handle('data_integracao', e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Data Ativação</label>
            <input type="date" className="form-control" value={form.data_ativacao || ''} onChange={e => handle('data_ativacao', e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Data Baixa</label>
            <input type="date" className="form-control" value={form.data_baixa || ''} onChange={e => handle('data_baixa', e.target.value)} />
          </div>
        </div>
        <div className="form-group">
          <label className="form-label">Observações</label>
          <textarea className="form-control" rows={2} value={form.observacoes || ''} onChange={e => handle('observacoes', e.target.value)} />
        </div>
...
export default function Pipeline() {
  ...
  useEffect(() => { carregar() }, [filtroStatus, filtroEmp, filtroResp])
  ...
  function editarUnidade(u) { setUnidadeEditando(u); setModalAberto(true) }
  function novaUnidade() { setUnidadeEditando(null); setModalAberto(true) }
  ...
  const unidadesFiltradas = unidades.filter(u => {
    ...
    return u.numero?.toLowerCase().includes(t) || u.empreendimento_nome?.toLowerCase().includes(t) || u.proprietario_nome?.toLowerCase().includes(t)
  })
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p>{unidades.length} unidades · {statCount('Ativo')} ativas</p>
        </div>
        {isAdmin && <button className="btn btn-primary" onClick={novaUnidade}>+ Nova Unidade</button>}
      </div>

      {/* Resumo por status */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        {STATUS_LIST.map(s => (
          <div key={s} onClick={() => setFiltroStatus(filtroStatus === s ? '' : s)}
            style={{ background: '#fff', border: `2px solid ${filtroStatus === s ? 'var(--primary)' : 'var(--neutral-200)'}`, borderRadius: 8, padding: '10px 16px', cursor: 'pointer', minWidth: 110, textAlign: 'center' }}>
        ...
        <div className="card-body" style={{ paddingTop: 16 }}>
          <div className="filters-row">
            <div>
              ...
              <input className="form-control" placeholder="Buscar unidade, empreendimento..." value={filtroTexto} onChange={e => setFiltroTexto(e.target.value)} style={{ minWidth: 240 }} />
            </div>
            <div>
              <label className="form-label">Status</label>
            ...
            <div style={{ alignSelf: 'flex-end' }}>
              <button className="btn btn-secondary" onClick={() => { setFiltroStatus(''); setFiltroEmp(''); setFiltroResp(''); setFiltroTexto('') }}>Limpar</button>
            </div>
          </div>
        </div>
      </div>
              ...
              <table>
                ...
                <tbody>
                  {unidadesFiltradas.length === 0 ? (
                    <tr><td colSpan={12} style={{ textAlign: 'center', padding: 48, color: 'var(--neutral-400)' }}>Nenhuma unidade encontrada</td></tr>
                    ...
                    <tr key={u.id}>
                      ...
                      <td><span className={`status-badge s-${u.status.toLowerCase()}`}>{labelStatus(u.status)}</span></td>
                      ...
                      {isAdmin && (
                        <td>
                          <div className="flex-gap">
                            ...
                            <button className="btn-icon" onClick={() => excluirUnidade(u.id)} title="Excluir" style={{ color: 'var(--danger)' }}>
