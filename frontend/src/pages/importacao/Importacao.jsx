...
const TIPOS = [
    {
        ...
        exemploCabecalho:
            "Empreendimento;Unidade;Proprietario;Data de Contrato;Data de Ativação;Data de Saida;Status da unidade;Comissao - Adm;BPO;Taxa - Enxoval;Nome da Indicação;Status - Pagamento Indicação;Responsável;Observação",
    },
    {
        id: "lancamentos",
...
function BadgeStatus({ ok }) {
    return ok ? <span style={s.badgeOk}>✓ encontrada</span> : <span style={s.badgeErr}>✗ ausente</span>;
}
        ...
        <div className="importacao-page" style={s.page}>
            ...
            <div className="importacao-info-box" style={s.infoBox}>
                ...
                <div style={s.infoSecao}>
                    ...
                    <div style={s.tagRow}>
                        {tipo.opcionais.map((c) => (
                            <span key={c} style={s.tagOpc}>
                                {c}
                            </span>
                        ))}
                    </div>
                ...
                <div style={s.exemploCabecalho}>
                    <strong>
                        Formato do cabeçalho (separador: <code>;</code>):
                    </strong>
            ...
            >
                {arquivo ? (
                    ...
                    <span>
                        Arraste um arquivo CSV aqui ou <u>clique para selecionar</u>
                    </span>
                )}
                <input ref={inputRef} type="file" accept=".csv,text/csv" style={{ display: "none" }} onChange={(e) => onArquivo(e.target.files[0])} />
            </div>
                        ...
                        <div>
                            ...
                            {validacao.colunas_opcionais_encontradas?.length ? (
                                validacao.colunas_opcionais_encontradas.map((c) => (
                                    <div key={c} style={s.colRow}>
                                        <code style={s.colNome}>{c}</code> <span style={s.badgeOk}>✓</span>
                                    </div>
                                ))
                            ) : (
            ...
            {resultado && !carregando && (
                <div style={s.resultadoBox}>
                    ...
                    <button style={s.btnNovo} onClick={resetar}>
...
const s = {
    ...
    tipoCardAtivo: { borderColor: "var(--primary)", background: "color-mix(in srgb, var(--primary) 10%, var(--surface))" },
    tipoLabel: { fontWeight: 600, fontSize: 15 },
    tipoDesc: { fontSize: 13, color: "var(--text-muted)" },
    ...
    dropzoneAtivo: { borderColor: "var(--primary)", background: "color-mix(in srgb, var(--primary) 10%, var(--surface))" },
    ...
    colNome: { fontSize: 13, background: "var(--neutral-100)", padding: "2px 6px", borderRadius: 4 },
    badgeOk: { background: "var(--success-light)", color: "var(--success)", borderRadius: 4, padding: "1px 7px", fontSize: 12 },
    badgeErr: { background: "var(--danger-light)", color: "var(--danger)", borderRadius: 4, padding: "1px 7px", fontSize: 12 },
    faltandoBox: {
        background: "var(--danger-light)",
        border: "1px solid color-mix(in srgb, var(--danger) 30%, var(--border))",
        color: "#dc2626",
