  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 500 }}>
        ...
        <div className="form-group"><label className="form-label">Endereço</label><input className="form-control" value={form.endereco} onChange={e => h('endereco', e.target.value)} /></div>
        <div className="form-grid">
          <div className="form-group"><label className="form-label">Cidade</label><input className="form-control" value={form.cidade} onChange={e => h('cidade', e.target.value)} /></div>
          <div className="form-group"><label className="form-label">Estado</label><input className="form-control" value={form.estado} onChange={e => h('estado', e.target.value)} maxLength={2} placeholder="PB" /></div>
        </div>
  ...
  return (
    <div>
      <div className="page-header">
        <div><h2>Empreendimentos</h2><p>{lista.length} cadastrados</p></div>
        ...
        <div className="card-body">
          {loading ? <div className="loading">Carregando...</div> : (
            <div className="table-container">
              <table>
                <thead><tr><th>Nome</th><th>Endereço</th><th>Cidade</th><th>Estado</th><th>Unidades</th><th>Ativas</th><th>Status</th><th>Ações</th></tr></thead>
                <tbody>
                  {lista.map(e => (
                    <tr key={e.id}>
                      ...
                      <td><span className="status-badge s-ativo">{e.unidades_ativas}</span></td>
                      <td><span className={`status-badge ${e.ativo ? 's-ativo' : 's-baixa'}`}>{e.ativo ? 'Ativo' : 'Inativo'}</span></td>
                      <td>
                        <div className="flex-gap">
                          <button className="btn-icon" onClick={() => { setEditando(e); setModal(true) }} title="Editar">
                          ...
                          <button className="btn-icon" style={{ color: 'var(--danger)' }} onClick={() => desativar(e.id)} title="Desativar">
      ...
      {modal && <Modal emp={editando} onClose={() => setModal(false)} onSave={() => { setModal(false); carregar() }} />}
    </div>
  )
}
